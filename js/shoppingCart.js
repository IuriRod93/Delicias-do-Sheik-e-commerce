...
const generateOrder = () => {
    ...
    let cartMessage = '*Essa mensagem será enviada para o WhatsApp da lanchonete.* \n' // Remover essa mensagem quando tiver um WhatsApp
    cartMessage += 'Boa noite! Gostaria de encomendar: \n'

    generatedCart.length > 0 && generatedCart.sort((a, b) => a.type < b.type ? -1 : a.type > b.type ? 1 : 0 )

    generatedCart.forEach(item => {
        cartMessage += '- ' + item.qtd + ' ' + item.name + '\n'
    })
    ...
    // Encodificando a mensagem para URL
    const messageEncoded = encodeURIComponent(cartMessage);

    // Substitua "seuNumero" pelo número do WhatsApp, incluindo o código do país
    const phoneNumber = '5521993079682'; // Exemplo: '5521981956112'
    const whatsappUrl = `https://wa.me/${phoneNumber}?text=${messageEncoded}`;

    // Abrindo o WhatsApp com a mensagem gerada
